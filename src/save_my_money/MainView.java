...
{  
    ...
    private static final int SCHENDULE = 4;
    ...
    /**
     * Inicializate the class, load the user data and configure 
     * the visual interface.Data will be chargued with an id, that the user will write on a 
     * JOptionPane. If id introducted not refence any data, it creates a new directory
     * in SaveMyMoney directory, and put in a new .txt to save the future data.
     */
    public MainView()
    {
        id = chargeData();
    ...
    /**
     * If user clicks on addIncOrExpButton, a new window will be opened to 
     * add new data. 
     * If user clicks on If user clicks on, btnGeneral, expensesButton,
     * incomeButton, balanceButton or schenduleButton, the program will chage 
     * the visual panel dependding on the button selected.
    ...
    /** 
     *  Chargue the user data to analyze and in the program. 
     *  Data will be chargued with an id, that the user will write on a 
     *  JOptionPane.
     * @return 
     */
    ...
    public class GeneralPanel extends JPanel 
    {
         ...
         * @param panelToDraw Type the panel to draw. There are four panel types; incomes,
         * expenses,general,balance and schendule.
         */
        ...
        /**
         * Draw the balance panel. It composes by a circle graph of the total balance,
         * a line graph of the balance evolution, and a bar horizontal graph of the
         * incomes and the expenses.
         * @param g The Graphics context in which to paint.
         */
        private void drawBalancePanel(Graphics g)
        {
            ...
            for(int i = 0; i<data.categoriesData.length; i++)
            {
                if(data.categoriesData[i].income != 0)
                {
                    ...
                    g.fillRect(120, lastIncomeDraw, (int)(data.categoriesData[i].income * multiplierBar), n);
                }
            }
            ...
            for(int i = data.categoriesData.length-1; i>=0; i--)
            {
                if(data.categoriesData[i].expense != 0)
                {
                    ...
                    g.setColor(data.categoriesData[i].color);
                    g.fillRect(120-expenseToDraw, lastIncomeDraw + (n*(sizeExpense-contExp)), expenseToDraw , n);
                }
            }
        }
        
        /**
         * Draw the expenses interface.It composes by a circle graph of the expenses 
         *  categories and a horizontal bar graph of expenses,ordered descendantly.
         * @param g The Graphics context in which to paint.
         */
        private void drawExpensePanel(Graphics g)
        {
            ...
            for(int i=0; i<panelData.categoriesData.length; i++)
            {
                if(i>13)
                    break;
        ...
        /**
         * Draw the general interface.It composes by a circle graph and
         * two horizontal bar graphs ordered descendantly, one for the incomes
         * and the other for the expenses.
         * @param g The Graphics context in which to paint.
         */
        ...
        /**
         * Draw the incomes interface.It composes by a circle graph of the incomes
         * categoreies and a horizontal bar graph of incomes,ordered descendantly.
         * @param g The Graphics context in which to paint.
         */
